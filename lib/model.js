 ...
 */
lastStepModifierFunctions.$set = function (obj, field, value, affectingIndex) {
  if (typeof value === 'function') {
    obj[field] = value(obj[field], affectingIndex);
  } else {
    obj[field] = value;
  }
};
...
function createModifierFunction (modifier) {
  return function (obj, field, value, affectingIndex = 0) {
    var fieldParts = typeof field === 'string' ? field.split('.') : field;

    if (fieldParts.length === 1) {
      lastStepModifierFunctions[modifier](obj, field, value, affectingIndex);
    } else {
      if (obj[fieldParts[0]] === undefined) {
        if (modifier === '$unset') { return; }   // Bad looking specific fix, needs to be generalized modifiers that behave like $unset are implemented
        obj[fieldParts[0]] = {};
      }
      modifierFunctions[modifier](obj[fieldParts[0]], fieldParts.slice(1), value, affectingIndex);
    }
  };
}
 ...
 */
function modify (obj, updateQuery, affectingIndex) {
  ...
  if (dollarFirstChars.length === 0) {
    ...
    modifiers.forEach(function (m) {
      ...
      keys.forEach(function (k) {
        modifierFunctions[m](newDoc, k, updateQuery[m][k], affectingIndex);
      });
    });
  }
